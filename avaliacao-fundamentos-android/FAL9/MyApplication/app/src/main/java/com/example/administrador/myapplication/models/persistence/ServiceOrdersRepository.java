...
public final class ServiceOrdersRepository {
    ...
    public List<ServiceOrder> findAll(ServiceOrder serviceOrder) {
        DatabaseHelper helper = new DatabaseHelper(AppUtil.CONTEXT);
        SQLiteDatabase db = helper.getReadableDatabase();
        ...
        if (serviceOrder.getActive() != null) {
            where.append(" AND ").append(ServiceOrderContract.ACTIVE).append(" = ? ");
            args.add(serviceOrder.getActive().toString());
        }

        Cursor cursor = db.query(ServiceOrderContract.TABLE,
                ServiceOrderContract.COLUNS,
                where.toString(),
                args.toArray(new String[args.size()]),
                null, null, null);
        List<ServiceOrder> users = ServiceOrderContract.bindList(cursor);
        db.close();
        helper.close();
        return users;
    }
